...
`acl.*` attributes; **Explore → Prometheus**, `acl_decisions_total` or `acl_otel_healthy_ratio` for
...
- **`unit: "1"` becomes a `_ratio` suffix.** The OTel-to-Prometheus convention turns a gauge whose
  unit is `1` into `<name>_ratio`, so `acl.node.info` arrives as `acl_node_info_ratio` and
  `acl.sessions.live` as `acl_sessions_live_ratio` - neither is a ratio. Thirteen metrics are
  affected, the base's and ours alike. The fix is on the producing side (an empty unit, or a UCUM
  annotation like `{session}`), and it belongs to whoever owns each gauge.
